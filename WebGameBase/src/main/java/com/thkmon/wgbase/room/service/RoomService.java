...
public class RoomService implements RoomConst {
	...
	public void handleDisconnection(Session session) throws MessageException, Exception {
		...
		if (gamerCount == 0) {
			RoomServiceUtil.destoryGameRoom(session);
		}
		
		// 연결 끊기면 턴 조정
		RoomData roomData = RoomServiceUtil.getRoomData(session);
		if (roomData == null) {
			return;
		}
		...
		UserSession userSession = RoomServiceUtil.getUserSession(session);
		if (userSession == null) {
			return;
		}
		...
		String userListString = RoomServiceUtil.getUserListString(session);
...
//		}
	...
	private String getSessionIdIfLogoutUser(Session newSession, String newNickName, boolean bForbidRevive) throws MessageException, Exception {
		...
		UserSessionList userSessionList = RoomServiceUtil.getUserSessionListBySession(newSession);
		if (userSessionList == null) {
			return null;
		}
	...
	private void handleRoomId(String messageValue, Session session) throws MessageException, Exception {
		...
		RoomServiceUtil.addUserSessionList(roomId, session);
		return;		
	}
	...
	private void handleStartGame(String messageValue, Session session) throws MessageException, Exception {
		...
		// 메시지 보내기
		String commonMsg = "CHAT|***** 게임을 시작합니다. *****" + "/+/" + "MESSAGE|" + "게임 시작!";
		sendMessageToAll(session, commonMsg, "");
	}
